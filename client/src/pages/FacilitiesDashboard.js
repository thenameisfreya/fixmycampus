...
const FacilitiesDashboard = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(p.x,p.y,Math.max(s,0.1),0,Math.PI*2);
        ctx.fillStyle=`rgba(${p.c.r},${p.c.g},${p.c.b},0.5)`;
        ctx.fill();
      });
  ...
  const addNote = () => {
    ...
    setNotes(prev => ({
      ...prev,
      [selected._id]: [...(prev[selected._id] || []), newNote]
    }));
    setNote('');
  };
      ...
      <div style={{ width:'340px', display:'flex', flexDirection:'column', background:'#fff', borderRight:'1px solid #e5e7eb', position:'relative', zIndex:5, overflow:'hidden' }}>
        ...
        <div style={{ padding:'16px 16px 12px', borderBottom:'1px solid #f0f0f0', flexShrink:0 }}>
          ...
          <div style={{ position:'relative', marginBottom:'10px' }}>
            ...
            <input
              type="text"
              placeholder="Search title, building, category..."
              value={search}
        ...
        {!selected ? (
          ...
          <>
            ...
            <div style={{ padding:'16px 22px', borderBottom:'1px solid rgba(255,255,255,0.08)', flexShrink:0 }}>
              ...
              <textarea
                placeholder="Add an internal note — only visible to facilities staff..."
                value={note}
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'12px' }}>
                <p style={{ color:'rgba(255,255,255,0.35)', fontSize:'11px', margin:0 }}>
                  {assignedTo
                    ...
                    : <span style={{ color:'rgba(255,255,255,0.35)' }}>Unassigned</span>
                  }
                </p>
